...
AmbientOcclusionVK::AmbientOcclusionVK(
    ...
    m_in_position_depth_attachment(i_in_position_attachment),
...
bool AmbientOcclusionVK::initialize()
{
    ...
    //SHADER STAGES
    {
        { // difuse
            ...
            VkShaderModule frag_module = m_runtime.m_shader_registry->loadShader("./shaders/ambient_occlusion.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
    ...
    //creamos textura de ruido para SSAO
    unsigned char* text_noise = new unsigned char[width * height * 4]; 

    for (uint32_t i = 0; i < width * height * 4; i++)
    {
        text_noise[i] = static_cast<unsigned char>(rand() % 255);
    }

    UtilsVK::TextureFromBuffer(*m_runtime.m_renderer->getDevice(), text_noise, width * height * 4 * sizeof(uint8_t), VK_FORMAT_R8G8B8A8_UNORM, width, height, m_text_noise);
...
void AmbientOcclusionVK::shutdown()
{
    RendererVK& renderer = *m_runtime.m_renderer;
...
VkCommandBuffer AmbientOcclusionVK::draw(const Frame& i_frame)
{
    RendererVK& renderer = *m_runtime.m_renderer;
...
void AmbientOcclusionVK::createFbo()
{
    RendererVK& renderer = *m_runtime.m_renderer;
    ...
    for (size_t i = 0; i < m_fbos.size(); i++)
    {
        ...
        attachments[0] = m_ssao_attachment.m_image_view; // Color attachment is the view of the swapchain image
...
void AmbientOcclusionVK::createRenderPass()
{
    RendererVK& renderer = *m_runtime.m_renderer;
    ...
    attachments[0].finalLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL; // CAMBIAMOS EL FINAL LAYOUT YA QUE NO SE TIENE QUE MOSTRAR
    ...
    subpass_description.pColorAttachments = &ssao_ref;
...
void AmbientOcclusionVK::createPipelines()
{
    RendererVK& renderer = *m_runtime.m_renderer;
...
void AmbientOcclusionVK::createDescriptorLayout()
{
    ...
    //textura de posicion + profundidad
    layout_bindings[1] = {};
...
void AmbientOcclusionVK::createDescriptors()
{
    ...
    for (uint32_t i = 0; i < m_runtime.m_renderer->getWindow().getImageCount(); i++)
    {
        ...
        VkDescriptorBufferInfo bkernel;
        bkernel.buffer = m_runtime.getKernelBuffer()[i];
        bkernel.offset = 0;
        bkernel.range = sizeof(KernelSSAO);
        ...
        image_infos[2].sampler = m_text_noise.m_sampler;
        image_infos[2].imageView = m_text_noise.m_image_view;
        image_infos[2].imageLayout = VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL;

        std::array<VkWriteDescriptorSet, 5> set_write;

        // Uniform buffer para parametros globales
        set_write[0] = {};
        ...
        // Imagen de posicion + profundidad
        set_write[1] = {};
        ...
        set_write[4].pImageInfo = nullptr;
        set_write[4].pBufferInfo = &bkernel;
